...
const Navbar = () => {
  ...
  const brandData = {
    column1: [
      ...
      'Carl F. Bucherer', 'Christiaan van der Klaauw', 'Chronoswiss', 'CIGA Design'
    ],
    column2: [
      'Corum', 'Cuervo y Sobrinos', 'CVSTOS', 'Czapek', 'Doxa', 'Eberhard & Co.',
      'Edouard Koehn', 'Edox', 'Ernest Borel', 'Eterna', 'Favre Leuba', 
      'Frederique Constant', 'Gerald Charles', 'Girard-Perregaux', 'Graham', 'Grand Seiko'
    ],
    column3: [
      'H. Moser & Cie.', 'HAUTLENCE', 'Hublot', 'Ikepod', 'IWC Schaffhausen', 'Jacob & Co.',
      'Jaeger-LeCoultre', 'Junghans', 'Laurent Ferrier', 'Longines', 'Louis Erard',
      'Louis Moinet', 'Luminox', 'Maserati', 'Maurice Lacroix', 'MeisterSinger'
    ],
    column4: [
      'Mühle-Glashütte', 'Nivada Grenchen', 'Nomos Glashutte', 'NORQAIN', 'Omega',
      'Oris', 'Panerai', 'Parmigiani', 'Perrelet', 'Rado', 'Raymond Weil', 'Ressence',
      'Seiko', 'Singer Reimagined', 'Speake-Marin', 'TAG Heuer'
    ],
    column5: [
      'Tissot', 'Titoni', 'Trilobe', 'Tudor', 'Tutima Glashütte', 'Ulysse Nardin',
      'Urwerk', 'Zenith', 'Zeppelin'
    ]
  };
    ...
    <nav className="navbar">
      ...
      <div className="navbar-bottom flex justify-center w-full">
        <ul className="bottom-links flex">
          ...
          >
            <Link to="/brands" className="text-sm tracking-wider font-medium hover:text-black">
              BRANDS
            </Link>
            
            {showBrandsDropdown && (
              <div className="brands-mega-dropdown">
                <div className="brands-header">
                  <h3>Our Watch Brands</h3>
                </div>
                
                <div className="brands-grid-container">
                  <div className="brands-column">
                    {brandData.column1.map((brand, index) => (
                      <Link key={index} to={`/brand/${brand.toLowerCase().replace(/\s+/g, '-')}`} className="brand-link">
                        {brand}
                      </Link>
                    ))}
                  </div>
                  ...
                  <div className="brands-column">
                    {brandData.column2.map((brand, index) => (
                      <Link key={index} to={`/brand/${brand.toLowerCase().replace(/\s+/g, '-')}`} className="brand-link">
                        {brand}
                      </Link>
                    ))}
                  ...
                  <div className="brands-column">
                    {brandData.column3.map((brand, index) => (
                      <Link key={index} to={`/brand/${brand.toLowerCase().replace(/\s+/g, '-')}`} className="brand-link">
                        {brand}
                      </Link>
                    ))}
                  ...
                  <div className="brands-column">
                    {brandData.column4.map((brand, index) => (
                      <Link key={index} to={`/brand/${brand.toLowerCase().replace(/\s+/g, '-')}`} className="brand-link">
                        {brand}
                      </Link>
                    ))}
                  ...
                  <div className="brands-column">
                    {brandData.column5.map((brand, index) => (
                      <Link key={index} to={`/brand/${brand.toLowerCase().replace(/\s+/g, '-')}`} className="brand-link">
                        {brand}
                      </Link>
                    ))}
                ...
                <div className="brands-footer">
                  <Link to="/all-brands" className="view-all-brands">
                    View All Brands →
                  </Link>
                </div>
              </div>
